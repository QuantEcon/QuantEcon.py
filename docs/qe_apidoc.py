...
def base_module_target(name):
    """
    Map a base-level module name to its automodule target.

    Most base-level public modules (e.g. ``quantecon.arma``) are
    deprecated compatibility shims around a private implementation
    module (e.g. ``quantecon._arma``). Document the implementation
    module, which carries the actual docstrings, rather than the shim.

    """
    if os.path.exists(os.path.join("..", "quantecon", "_" + name + ".py")):
        return "_" + name
    return name
    ...
    for mod in mod_names:
        ...
        with open(new_path, "w") as f:
            f.write(module_template.format(
                mod_name=name, equals="=" * len(name),
                target=base_module_target(name)))
