...
import warnings
from . import _estspec


__all__ = ['smooth', 'periodogram', 'ar_periodogram']
...
def __getattr__(name):
    if name not in __all__:
        raise AttributeError(
                "`quantecon.estspec` is deprecated and has no attribute "
                f"'{name}'."
            )

    warnings.warn(f"Please use `{name}` from the `quantecon` namespace, "
                  "the `quantecon.estspec` namespace is deprecated. You can use"
                  f" the following instead:\n `from quantecon import {name}`.",
                  category=DeprecationWarning, stacklevel=2)

    return getattr(_estspec, name)
